...
parser.add_argument('-output_prefix', dest = 'output_prefix', action='store', nargs = 1, type = str, help = 'name of the output file')
...
parser.add_argument('-num_indv', dest = 'num_indv', action='store', nargs = 1, default=100, type = int, help = 'number of haploid genomes')
...
def get_af(f_txt, mutn_types):
    ...
    for line in f_txt:
        ...
        if "#" not in line1:
            ...
            if "Genomes" in line1:
                check = 0
            if "Mutations" not in line1 and check==1:
                if "m" in line1:
                    line2 = line1.split()
                    if len(line2) == 9:
